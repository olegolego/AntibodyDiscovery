"""19-dim physicochemical (chemical) amino acid embedding — no GPU, no download."""
import sys
...
class AAChemEmbeddingAdapter:
    def __init__(self, spec: ToolSpec) -> None:
        self.spec = spec
        self._cache = MoleculeResultCache(tool_id="aa_chem_embedding", tool_version=spec.version)
    ...
    async def invoke(self, inputs: dict[str, Any], run_ctx: RunContext) -> dict[str, Any]:
        ...
        n = len(sequences)
        await run_ctx.alog(f"AA chemical embedding: {n} sequence(s), pool={pool_mode}, dim=19")
        ...
        if outputs.get("error"):
            raise RuntimeError(f"aa_chem_embedding failed: {outputs['error']}")

        await self._cache.put(cache_key, outputs, run_id=run_ctx.run_id, node_id=run_ctx.node_id)
        await run_ctx.alog(f"Done — {outputs.get('n', n)} pair(s), 19-dim physicochemical vectors")
        return outputs
